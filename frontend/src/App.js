...
function App() {
  ...
  const handleSelectUser = (user) => {
    ...
    // Keep track of all users for location sharing feature
    setAllUsers(prev => {
      ...
      <div className="min-h-screen bg-gray-100">
          ...
          {!currentUser ? (
            ...
            <div className="mt-2 sm:mt-4">
              ...
              <div style={{ display: activeTab === 'location' ? 'block' : 'none' }}>
                <SimulatedUsers currentUser={currentUser} allUsers={allUsers} />
                <LocationSharing currentUser={currentUser} allUsers={allUsers} />
              </div>
            </div>
          )}
        </main>
        ...
        {currentUser && (
          <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around z-10">
            ...
            <button
              onClick={() => setActiveTab('location')}
              className={`flex-1 py-3 text-xs ${activeTab === 'location' ? 'text-blue-600' : 'text-gray-500'}`}
            >
              Location
            </button>
          </div>
